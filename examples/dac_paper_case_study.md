...
For comparison:
- DirectChat single-shot baseline (Story2Proposal paper): **3.96**
- Story2Proposal (original framework, mixed backbones): **6.15**
- **Lunheng final: 8.66**
...
## Key Methodological Wins (vs. Single Reviewer)

A single Claude reviewer (`lunheng-quick` baseline) on the same R0 paper scored **7.6 / 10**. The 4 dimensions where the multi-agent setup beat the single reviewer:

| Dimension | Single reviewer | Lunheng |
|-----------|-----------------|---------|
...
All raw agent outputs are preserved in [`examples/dac_artifacts/`](dac_artifacts/) (TODO: anonymize and upload):
- `blueprint_round1.md`
